...
// Constructor with references to dlo and rigid bodies
CollisionHandler::CollisionHandler(pbd_object::Dlo &dlo, 
...
void CollisionHandler::updateAABB(pbd_object::Dlo &dlo_,
	...
	{
		// const Eigen::Matrix<Real,Eigen::Dynamic,3> *pos_ptr = dlo_.getPosPtr();
		...
		co->m_aabb.m_p[0] = (*pos_ptr)[0];
    	co->m_aabb.m_p[1] = (*pos_ptr)[0];
		
        // for (int j = 1; j < pos_ptr->rows(); j++)
		// {
		// 	updateAABB(pos_ptr->row(j).transpose(), co->m_aabb);
		// }
		for (int j = 1; j < pos_ptr->size(); j++)
		{
			updateAABB((*pos_ptr)[j], co->m_aabb);
		}
	}
...
void CollisionHandler::collisionDetection(){
	...
	#pragma omp parallel default(shared)
	{
		...
		// We have Bounding Sphere Hierachy (BSH) for dlo
		// Axis Aligned Bounding Box for all (i.e., dlo and rigid bodies)
		...
		// Update each of the collision objects the AABB's and the dlo's m_bvh
		for (int i = 0; i < (int)m_collisionObjects.size(); i++)
		{
			CollisionHandler::CollisionObject *co = m_collisionObjects[i];
			updateAABB(dlo_, rigid_bodies_, co);

			/* We only update the dlo's m_bvh but not other (rigid) objects 
		...
		for (int i = 0; i < (int)coPairs.size(); i++)
		{
			...
			// Enforce that co1 can be either dlo or rigid body, but co2 can only be a rigid body.
			...
			// Collision detection between a dlo and a rigid body
					 ...
					 (co1->m_testMesh))
			{
				...
				// Set (Calculate) the CONTACT's restitution & friction coefficients
				// Assume the dlo's restitution coeff = 1 and friction coeff = 0.0
				...
				// and co2's distance and collisionTest functions.
				// If they indeed in collision, add the related ContactData to contacts_mt.
				collisionDetectionRBSolid(dlo_,
										  co1,
										  rb2,
										  co2,
										  restitutionCoeff,
...
void CollisionHandler::collisionDetectionRBSolid(pbd_object::Dlo &dlo_,
								CollisionObject *co1, 
								RigidBodySceneLoader::RigidBodyData &rb2, 
								CollisionObject *co2, 
								const Real &restitutionCoeff, 
								...
								std::vector< std::vector<ContactData> > &contacts_mt)
{
	...
	std::function<bool(unsigned int, unsigned int)> predicate = [&](unsigned int node_index, unsigned int depth)
	{
		...
		// Test if center of bounding sphere of dlo intersects with rigid body's AABB
		if (dist < bs.r())
		{
			// Test if distance of center of bounding sphere to collision object is smaller than the radius
	...
	std::function<void(unsigned int, unsigned int)> cb = [&](unsigned int node_index, unsigned int depth)
	{
		...
		for (auto i = node.begin; i < node.begin + node.n; ++i)
		{
			...
			const Eigen::Matrix<Real, 3, 1> &x_w = (*pos_ptr)[index];
...
{
	// This function finds the minimum distances from dlo_ to the rigid bodies.
	...
	// Collect only rigid bodies and a single dlo object.
	std::vector<CollisionObject*> rigidBodies;
	CollisionObject* dlo = nullptr;
	...
	#pragma omp parallel default(shared)
	{
		...
		for (int i = 0; i < rigidBodies.size(); i++) 
		{
			...
			computeMinDistanceRBSolid(dlo_, dlo, rbData, rigidBody, minDistanceData);
...
void CollisionHandler::computeMinDistanceRBSolid(pbd_object::Dlo &dlo_,
                                                 CollisionObject *co1, 
                                                 RigidBodySceneLoader::RigidBodyData &rb2, 
                                                 CollisionObject *co2,
                                                 MinDistanceData& minDistanceData)
{
	//find the minimum distance between a dlo object and a given rigid body 
    ...
    auto callback = [&](unsigned int node_index, unsigned int) {
        ...
        for (auto i = node.begin; i < node.begin + node.n; ++i) {
            unsigned int index = bvh.entity(i);
            // const Eigen::Matrix<Real, 3, 1>& x_w = pos_ptr->row(index).transpose(); // point on dlo
			const Eigen::Matrix<Real, 3, 1>& x_w = (*pos_ptr)[index]; // point on dlo
														...
														const Real &frictionCoeffDynamic)
{
	...
	// const Eigen::Matrix<Real,Eigen::Dynamic,3> *pos_ptr = dlo_.getPosPtr();
	const std::vector<Eigen::Matrix<Real,3,1>> *pos_ptr = dlo_.getPosPtr();
	// const Eigen::Matrix<Real,Eigen::Dynamic,3> *vel_ptr = dlo_.getVelPtr();
	Eigen::Matrix<Real,3,Eigen::Dynamic> *vel_ptr = dlo_.getVelPtr();
	// const Eigen::Matrix<Real,1,Eigen::Dynamic> *inv_mass_ptr = dlo_.getInvMassPtr();
	const std::vector<Real> *inv_mass_ptr = dlo_.getInvMassPtr();
	...
	const Eigen::Matrix<Real, 3, 1> &x1 = (*pos_ptr)[m_bodies[0]];
														...
														const Real &dt)
{
	...
	// const Eigen::Matrix<Real,Eigen::Dynamic,3> *pos_ptr = dlo_.getPosPtr();
	std::vector<Eigen::Matrix<Real,3,1>> *pos_ptr = dlo_.getPosPtr();
	// const Eigen::Matrix<Real,Eigen::Dynamic,3> *prev_pos_ptr = dlo_.getPrevPosPtr();
	const std::vector<Eigen::Matrix<Real,3,1>> *prev_pos_ptr = dlo_.getPrevPosPtr();
	// const Eigen::Matrix<Real,1,Eigen::Dynamic> *inv_mass_ptr = dlo_.getInvMassPtr();
	const std::vector<Real> *inv_mass_ptr = dlo_.getInvMassPtr();
	...
	Eigen::Matrix<Real, 3, 1> corr_x1; // linear position correction for the particle of the dlo
														...
														const Real &dt)
{
	...
	// Hence we will ignore the corrections for the rigid body since we assume they are non-dynamic
	// TODO: Implement this properly when the rigid bodies are dynamic.
	Eigen::Matrix<Real, 3, 1> corr_v1; // linear velocity correction for the particle of the dlo
