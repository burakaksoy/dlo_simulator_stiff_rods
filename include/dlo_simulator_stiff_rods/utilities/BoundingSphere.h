...
namespace utilities
{
	...
	public:
		 ...
		 */
		BoundingSphere() : m_x(Eigen::Matrix<Real, 3, 1>::Zero()), m_r(0.0) {}
		 ...
		 */
		BoundingSphere(Eigen::Matrix<Real, 3, 1> const& x, const Real r) : m_x(x), m_r(r) {}
		 ...
		 */
		BoundingSphere(const Eigen::Matrix<Real, 3, 1>& a)
		{
			m_x = a;
			m_r = 0.0;
		}
		 ...
		 */
		BoundingSphere(const Eigen::Matrix<Real, 3, 1>& a, const Eigen::Matrix<Real, 3, 1>& b)
		{
			const Eigen::Matrix<Real, 3, 1> ba = b - a;
		 ...
		 */
		BoundingSphere(const Eigen::Matrix<Real, 3, 1>& a, const Eigen::Matrix<Real, 3, 1>& b, const Eigen::Matrix<Real, 3, 1>& c)
		{
			const Eigen::Matrix<Real, 3, 1> ba = b - a;
			const Eigen::Matrix<Real, 3, 1> ca = c - a;
			const Eigen::Matrix<Real, 3, 1> baxca = ba.cross(ca);
		 ...
		 */
		BoundingSphere(const Eigen::Matrix<Real, 3, 1>& a, const Eigen::Matrix<Real, 3, 1>& b, const Eigen::Matrix<Real, 3, 1>& c, const Eigen::Matrix<Real, 3, 1>& d)
		{
			const Eigen::Matrix<Real, 3, 1> ba = b - a;
			const Eigen::Matrix<Real, 3, 1> ca = c - a;
			const Eigen::Matrix<Real, 3, 1> da = d - a;
			Eigen::Matrix<Real, 3, 1> r;
			Eigen::Matrix<Real, 3, 3> T;
			T << ba[0], ba[1], ba[2],
				ca[0], ca[1], ca[2],
		 ...
		 */
		BoundingSphere(const std::vector<Eigen::Matrix<Real, 3, 1>>& p)
		{
			m_r = 0;
			m_x.setZero();
			setPoints(p);
		 ...
		 */
		Eigen::Matrix<Real, 3, 1> const& x() const { return m_x; }
		 ...
		 */
		Eigen::Matrix<Real, 3, 1>& x() { return m_x; }
		 ...
		 * \param p vertices of the points
		 */
		void setPoints(const std::vector<Eigen::Matrix<Real, 3, 1>>& p)
		{
			//remove duplicates
			std::vector<Eigen::Matrix<Real, 3, 1>> v(p);
			std::sort(v.begin(), v.end(), [](const Eigen::Matrix<Real, 3, 1>& a, const Eigen::Matrix<Real, 3, 1>& b)
			{
			...
			v.erase(std::unique(v.begin(), v.end(), [](Eigen::Matrix<Real, 3, 1>& a, Eigen::Matrix<Real, 3, 1>& b) { return a.isApprox(b); }), v.end());
			...
			for (int i = n - 1; i > 0; i--)
			{
				const Eigen::Matrix<Real, 3, 1> epsilon_vec = epsilon * Eigen::Matrix<Real, 3, 1>::Random();
		 ...
		 */
		bool contains(Eigen::Matrix<Real, 3, 1> const& other) const
		{
	...
	private:
		 ...
		 */
		BoundingSphere ses3(int n, std::vector<Eigen::Matrix<Real, 3, 1>>& p, Eigen::Matrix<Real, 3, 1>& q1, Eigen::Matrix<Real, 3, 1>& q2, Eigen::Matrix<Real, 3, 1>& q3)
		{
			BoundingSphere S(q1, q2, q3);

			for (int i = 0; i < n; i++)
			{
				Eigen::Matrix<Real, 3, 1> d = p[i] - S.x();
				if (d.squaredNorm() > S.r()* S.r())
		 ...
		 * \return		smallest enclosing sphere
		 */
		BoundingSphere ses2(int n, std::vector<Eigen::Matrix<Real, 3, 1>>& p, Eigen::Matrix<Real, 3, 1>& q1, Eigen::Matrix<Real, 3, 1>& q2)
		{
		 ...
		 * \return		smallest enclosing sphere
		 */
		BoundingSphere ses1(int n, std::vector<Eigen::Matrix<Real, 3, 1>>& p, Eigen::Matrix<Real, 3, 1>& q1)
		{
