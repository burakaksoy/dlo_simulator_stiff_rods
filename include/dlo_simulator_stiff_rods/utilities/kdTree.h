...
namespace utilities
{
	...
	public:
		...
		void traverse_breadth_first(TraversalPredicate const& pred, 
		...
		void traverse_breadth_first_parallel(TraversalPredicate pred, 
	...
	protected:
		...
		void construct(unsigned int node, 
					   Eigen::AlignedBox<Real, 3> const& box,
					   unsigned int b, 
					   unsigned int n);

		void traverse_depth_first(unsigned int node, 
								  unsigned int depth,
		...
		virtual Eigen::Matrix<Real, 3, 1> const& entity_position(unsigned int i) const = 0;

		virtual void compute_hull(unsigned int b, 
								  unsigned int n, 
								  HullType& hull) const = 0;
		
		virtual void compute_hull_approx(unsigned int b, 
										 unsigned int n, 
										 HullType& hull) const
		{
