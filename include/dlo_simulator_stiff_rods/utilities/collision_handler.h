...
#include "dlo_simulator_stiff_rods/utilities/AABB.h"
...
namespace utilities
{
    ...
    public:
        ...
        struct RigidBodyContactConstraint
        {
            ...
            bool solvePositionConstraint(pbd_object::Dlo &dlo_,
                                        std::vector<utilities::RigidBodySceneLoader::RigidBodyData> &rigid_bodies_,
                                        const Real &dt);

            bool solveVelocityConstraint(pbd_object::Dlo &dlo_,
                                        std::vector<utilities::RigidBodySceneLoader::RigidBodyData> &rigid_bodies_,
                                        const Real &dt);
        };
        ...
        CollisionHandler(pbd_object::Dlo &dlo, 
        ...
        // (dlo_ and rigid_bodies_  are used to read the current state of the 
        // collision object, whichever corresponds to the co)
        void updateAABB(pbd_object::Dlo &dlo_,
                                           ...
                                           const Real frictionCoeffDynamic)
        {
            ...
            const bool res = cc.initConstraint(dlo_,
                                               rigid_bodies_,
                                               rbIndex1, 
                                               rbIndex2, 
                                               cp1, 
        ...
        void addCollisionObjectWithoutGeometry(const unsigned int bodyIndex, 
                                                const unsigned int bodyType,
                                                const Eigen::Matrix<Real,3,1> *vertices,
                                                const unsigned int numVertices,
                                                const bool testMesh /*= true*/)
        {
            CollisionHandler::CollisionObject *co = new CollisionHandler::CollisionObject();
        ...
        void solveContactPositionConstraints(const Real &dt)
        {
            ...
            {
                m_rigidBodyContactConstraints[i].solvePositionConstraint(dlo_,
                                                                         rigid_bodies_, dt);
            }
            for (unsigned int i = 0; i < m_particleRigidBodyContactConstraints.size(); i++)
            {
                m_particleRigidBodyContactConstraints[i].solvePositionConstraint(dlo_,
                                                                                 rigid_bodies_, dt);
            }
        }
        ...
        void solveContactVelocityConstraints(const Real &dt)
        {
            for (unsigned int i = 0; i < m_rigidBodyContactConstraints.size(); i++)
            {
                m_rigidBodyContactConstraints[i].solveVelocityConstraint(dlo_,
                                                                         rigid_bodies_, dt);
            }
            for (unsigned int i = 0; i < m_particleRigidBodyContactConstraints.size(); i++)
            {
                m_particleRigidBodyContactConstraints[i].solveVelocityConstraint(dlo_,
                                                                                 rigid_bodies_, dt);
            }
        }
    ...
    private:
		...
		void *m_contactCBUserData; // dlo_simulator is passed here with this user data
		...
		void collisionDetectionRBSolid(pbd_object::Dlo &dlo_,
									   CollisionObject *co1, 
									   RigidBodySceneLoader::RigidBodyData &rb2, 
									   CollisionObject *co2, 
									   const Real &restitutionCoeff, 
        ...
        void computeMinDistanceRBSolid(pbd_object::Dlo &dlo_,
                                       CollisionObject *co1, 
                                       RigidBodySceneLoader::RigidBodyData &rb2, 
                                       CollisionObject *co2,
