...
KDTree<HullType>::construct()
{
	...
	// std::iota: Creates a range of sequentially increasing values. 
	// For each element in the range [first,last) assigns value 
	// and increments value as if by ++value.

	// Parameters:
	// m_lst.begin(): Start of range. That is m_lst vector's beginning index
	// m_lst.end(): End of range. m_lst vector's last index
	// 0: Starting value.
	std::iota(m_lst.begin(), m_lst.end(), 0);

	// Determine bounding box of considered domain.
	auto box = Eigen::AlignedBox<Real, 3>{};
...
template<typename HullType> void
KDTree<HullType>::construct(unsigned int node, 
							Eigen::AlignedBox<Real, 3> const& box, 
							unsigned int b,
							unsigned int n)
{
...
template<typename HullType> void
KDTree<HullType>::traverse_depth_first(TraversalPredicate pred, 
